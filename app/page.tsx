...
function formulaText(play: PlayKey) {
  ...
  return `${playLabel(play)}使用独立公式。模型先进行大规模排名公式搜索，再针对高连断状态追加每档20,000—30,000套候选修复公式；按全历史最长连断、近三年最长连断、超限惩罚和命中率依次比较。开奖号必须为组六且三个不同数字全部入池才算命中，组三只单独标记覆盖。全历史结果属于训练回放。`;
}
