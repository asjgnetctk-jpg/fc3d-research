...
const auditPrefix = window.LotteryGame?.id === "pl3" ? "pl3-" : "";
let payload;
let activePlay = "dan";
let showAll = false;
let searchQuery = "";
...
async function load() {
  ...
  try {
    const dataFile = window.LotteryGame?.file("data.json") ?? "data.json";
    const response = await fetch(`${VERIFIED_DATA_ROOT}/${dataFile}?t=${Date.now()}`, { cache: "no-store" });
