  ...
  return `
    <article class="history-row">
      ...
      <div class="history-data">
        <div><span>推荐</span><strong>胆${row.dan} · ${row.pool7} · ${row.shapePlay} ${(row.group3Probability * 100).toFixed(1)}%</strong></div>
      ...
      <div class="history-result">
        ...
        <div><span>组三</span>${badge(row.shapeHit)}<small>断${row.shapeMissStreak}</small></div>
      </div>
    </article>
  `;
}
...
function render(data) {
  ...
  $("#lock-date").textContent = `数据更新至 ${data.trainingUpdatedThrough}`;
  ...
  $("#group3-probability").textContent =
    `模型参考概率 ${(data.recommendation.group3Probability * 100).toFixed(1)}%`;
