...
function render(data) {
  ...
  $("#kill-digits").innerHTML = [...data.recommendation.kills].map((digit) => `<b>${digit}</b>`).join(""); $("#locked-through").textContent = data.recommendation.validFor;
  const firstTitle = data.calculationMode === "daily" ? "近5年逐期滚动" : "近5年逐年锁定";
  const firstNote = data.calculationMode === "daily" ? `每期使用此前${data.trainingWindowYears}年数据` : `每年使用此前${data.trainingWindowYears}年数据`;
  $("#metrics").innerHTML = metricCard(firstTitle, data.metrics.rollingAll, firstNote) + metricCard("最后一年独立确认", data.metrics.independent, "2025-07-30—2026-07-29") + metricCard("上线后验证", data.metrics.live, "2026-07-30起，仅作后续核验");
