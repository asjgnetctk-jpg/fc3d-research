...
export function createEvaluator(rows) {
  ...
  function metricsForPlay(candidate, play, startIndex, endIndex) {
    const isDan = play === "dan";
    const size = isDan ? 1 : Number(play.replace("pool", ""));
    let hits = 0;
    let miss = 0;
    let maxMiss = 0;
    for (let index = startIndex; index < endIndex; index += 1) {
      ...
      const unique = [...new Set(rows[index].digits)];
      const hit = isDan
        ? unique.includes(selected[0])
        : unique.length === 3 && unique.every((digit) => selected.includes(digit));
      if (hit) {
        hits += 1;
        miss = 0;
      } else {
  ...
  return { metrics, metricsForPlay, pool };
}
...
export function compareHighHit(left, right) {
  return right.metrics.rate - left.metrics.rate || left.metrics.maxMiss - right.metrics.maxMiss;
}
